...
public class DateHelper {
   ...
   */
  private static final int[] DAYS_PER_MONTH = new int[] { 31, 28, 31, 30, 31,
   ...
   * 
   * @param date
   *          is a date to operate with.
   */
  public DateHelper(Date date) {
    this.date = date;
    this.firstDayOfWeek = Integer.valueOf(((CalendarConstants) GWT
        .create(CalendarConstants.class)).firstDayOfWeek());
  }
   ...
   *          is a year number.
   * @param month
   *          is a month number (from 0 to 11).
   * @param day
   *          is a day number.
   * @param hours
   *          is a hours number (from 0 to 23).
   * @param minutes
   *          is a minutes number.
   * @param seconds
   *          is a seconds number.
   */
  public DateHelper(int year, int month, int day, int hours, int minutes,
      int seconds) {
   ...
   * <p>
   * Usually a week starts from Sunday (0), but in some localizations it can
   * start from Monday. In this case all numbers will be shifted, i.e. Monday
  ...
  /**
   * This method gets a week number starting from the first of January (the
   * first week).
   * 
  ...
  public Date getLastDayOfWeek() {
    return new Date(date.getYear(), date.getMonth(), date.getDate() + 6
        - getDayOfWeek());
  }
  ...
  public int getDaysFrom(Date startDate) {
    return (int) Math.round((double) (trim().getTime() - new DateHelper(
        startDate).trim().getTime()) / 86400000);
  }
  ...
  public Date getLastDayOfMonth() {
    return new Date(date.getYear(), date.getMonth(),
        new DateHelper(date).getNumberOfDaysInMonth());
  }
